object frmCadProduto: TfrmCadProduto
  Left = 171
  Top = 113
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Cadastro de Produtos'
  ClientHeight = 464
  ClientWidth = 638
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  FormStyle = fsMDIChild
  OldCreateOrder = False
  Position = poScreenCenter
  Visible = True
  WindowState = wsMaximized
  OnActivate = FormActivate
  OnClose = FormClose
  PixelsPerInch = 96
  TextHeight = 13
  object pnlSuperior: TPanel
    Left = 0
    Top = 0
    Width = 638
    Height = 32
    Align = alTop
    Alignment = taLeftJustify
    Color = clBlue
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -19
    Font.Name = 'MS Sans Serif'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    object DBText2: TDBText
      Left = 12
      Top = 4
      Width = 91
      Height = 24
      AutoSize = True
      DataField = 'PRO_NMPROD'
      DataSource = dsProdutos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWhite
      Font.Height = -19
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold, fsItalic]
      ParentFont = False
      Transparent = True
    end
  end
  object Panel1: TPanel
    Left = 0
    Top = 32
    Width = 638
    Height = 391
    Align = alClient
    TabOrder = 1
    object Label1: TLabel
      Left = 12
      Top = 4
      Width = 90
      Height = 13
      Caption = 'C'#243'digo Fornecedor'
      Transparent = True
    end
    object Label2: TLabel
      Left = 12
      Top = 44
      Width = 73
      Height = 13
      Caption = 'C'#243'digo Produto'
      FocusControl = dbeCDPROD
      Transparent = True
    end
    object Label3: TLabel
      Left = 152
      Top = 45
      Width = 52
      Height = 13
      Caption = 'Refer'#234'ncia'
      FocusControl = dbeCDREFE
      Transparent = True
    end
    object Label4: TLabel
      Left = 12
      Top = 80
      Width = 48
      Height = 13
      Caption = 'Descri'#231#227'o'
      FocusControl = dbeNMPROD
      Transparent = True
    end
    object Label5: TLabel
      Left = 12
      Top = 118
      Width = 50
      Height = 13
      Caption = 'Fabricante'
      Transparent = True
    end
    object Label6: TLabel
      Left = 199
      Top = 118
      Width = 40
      Height = 13
      Caption = 'Unidade'
      Transparent = True
    end
    object Label7: TLabel
      Left = 381
      Top = 117
      Width = 108
      Height = 13
      Caption = 'Forma Armazenamento'
      Transparent = True
    end
    object Label8: TLabel
      Left = 12
      Top = 155
      Width = 103
      Height = 13
      Caption = 'Unid/Armazenamento'
      FocusControl = DBEdit4
      Transparent = True
    end
    object Label9: TLabel
      Left = 380
      Top = 155
      Width = 29
      Height = 13
      Caption = 'Grupo'
      Transparent = True
    end
    object Label10: TLabel
      Left = 197
      Top = 155
      Width = 31
      Height = 13
      Caption = 'Se'#231#227'o'
      Transparent = True
    end
    object Label11: TLabel
      Left = 12
      Top = 193
      Width = 69
      Height = 13
      Caption = 'Valor de Custo'
      FocusControl = dbeVLCUST
      Transparent = True
    end
    object Label12: TLabel
      Left = 12
      Top = 231
      Width = 80
      Height = 13
      Caption = 'Percentual ICMS'
      FocusControl = DBEdit6
      Transparent = True
    end
    object Label13: TLabel
      Left = 200
      Top = 231
      Width = 82
      Height = 13
      Caption = 'Percentual de IPI'
      FocusControl = DBEdit7
      Transparent = True
    end
    object Label14: TLabel
      Left = 12
      Top = 267
      Width = 77
      Height = 13
      Caption = 'Encargo Sociais'
      FocusControl = DBEdit8
      Transparent = True
    end
    object Label15: TLabel
      Left = 200
      Top = 270
      Width = 97
      Height = 13
      Caption = 'Encargo Financeiros'
      FocusControl = DBEdit9
      Transparent = True
    end
    object Label16: TLabel
      Left = 12
      Top = 306
      Width = 83
      Height = 13
      Caption = 'Margem de Lucro'
      FocusControl = DBEdit10
      Transparent = True
    end
    object Label17: TLabel
      Left = 12
      Top = 343
      Width = 73
      Height = 13
      Caption = 'Valor de Venda'
      FocusControl = dbeVLVEND
      Transparent = True
    end
    object cmbCDFORN: TDBLookupComboBox
      Left = 12
      Top = 20
      Width = 134
      Height = 21
      DataField = 'PRO_CDFORN'
      DataSource = dsProdutos
      KeyField = 'FOR_CDFORN'
      ListField = 'FOR_NMFORN'
      ListSource = dsFornecedor
      TabOrder = 0
      OnExit = cmbCDFORNExit
    end
    object dbeCDPROD: TDBEdit
      Left = 13
      Top = 58
      Width = 134
      Height = 21
      TabStop = False
      DataField = 'PRO_CDPROD'
      DataSource = dsProdutos
      ReadOnly = True
      TabOrder = 1
    end
    object dbeCDREFE: TDBEdit
      Left = 152
      Top = 58
      Width = 173
      Height = 21
      CharCase = ecUpperCase
      DataField = 'PRO_CDREFE'
      DataSource = dsProdutos
      TabOrder = 2
    end
    object dbeNMPROD: TDBEdit
      Left = 12
      Top = 95
      Width = 502
      Height = 21
      CharCase = ecUpperCase
      DataField = 'PRO_NMPROD'
      DataSource = dsProdutos
      TabOrder = 3
      OnExit = dbeNMPRODExit
      OnKeyPress = dbeNMPRODKeyPress
    end
    object cmbCDFABR: TDBLookupComboBox
      Left = 12
      Top = 131
      Width = 134
      Height = 21
      DataField = 'PRO_CDFABR'
      DataSource = dsProdutos
      KeyField = 'FOR_CDFORN'
      ListField = 'FOR_NMFORN'
      ListSource = dsFornecedor
      TabOrder = 4
      OnExit = cmbCDFABRExit
    end
    object cmbCDUNID: TDBLookupComboBox
      Left = 198
      Top = 131
      Width = 134
      Height = 21
      DataField = 'PRO_CDUNID'
      DataSource = dsProdutos
      KeyField = 'UNI_CDUNID'
      ListField = 'UNI_NMUNID'
      ListSource = dsUnidade
      TabOrder = 5
      OnExit = cmbCDUNIDExit
    end
    object cmbFARMA: TDBLookupComboBox
      Left = 380
      Top = 131
      Width = 134
      Height = 21
      DataField = 'PRO_FMARMA'
      DataSource = dsProdutos
      KeyField = 'UNI_CDUNID'
      ListField = 'UNI_NMUNID'
      ListSource = dsUnidade
      TabOrder = 6
      OnExit = cmbFARMAExit
    end
    object DBEdit4: TDBEdit
      Left = 12
      Top = 170
      Width = 134
      Height = 21
      DataField = 'PRO_QTARMA'
      DataSource = dsProdutos
      TabOrder = 7
    end
    object cmbCDSECA: TDBLookupComboBox
      Left = 199
      Top = 168
      Width = 134
      Height = 21
      DataField = 'PRO_CDSECA'
      DataSource = dsProdutos
      KeyField = 'SEC_CDSECA'
      ListField = 'SEC_NMSECA'
      ListSource = dsSecoes
      TabOrder = 8
      OnExit = cmbCDSECAExit
    end
    object dbeVLCUST: TDBEdit
      Left = 12
      Top = 208
      Width = 134
      Height = 21
      DataField = 'PRO_VLCUST'
      DataSource = dsProdutos
      TabOrder = 11
    end
    object DBEdit6: TDBEdit
      Left = 12
      Top = 244
      Width = 134
      Height = 21
      DataField = 'PRO_PEICMS'
      DataSource = dsProdutos
      TabOrder = 12
    end
    object DBEdit7: TDBEdit
      Left = 200
      Top = 244
      Width = 134
      Height = 21
      DataField = 'PRO_PERIPI'
      DataSource = dsProdutos
      TabOrder = 13
    end
    object DBEdit8: TDBEdit
      Left = 12
      Top = 284
      Width = 134
      Height = 21
      DataField = 'PRO_PESOCI'
      DataSource = dsProdutos
      TabOrder = 14
    end
    object DBEdit9: TDBEdit
      Left = 200
      Top = 284
      Width = 134
      Height = 21
      DataField = 'PRO_PEFINA'
      DataSource = dsProdutos
      TabOrder = 15
    end
    object DBEdit10: TDBEdit
      Left = 12
      Top = 320
      Width = 134
      Height = 21
      DataField = 'PRO_VLMACU'
      DataSource = dsProdutos
      TabOrder = 16
    end
    object dbeVLVEND: TDBEdit
      Left = 12
      Top = 358
      Width = 134
      Height = 21
      DataField = 'PRO_VLVEND'
      DataSource = dsProdutos
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clRed
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 17
    end
    object edtNMGRUP: TEdit
      Left = 381
      Top = 168
      Width = 134
      Height = 21
      ReadOnly = True
      TabOrder = 9
      OnEnter = edtNMGRUPEnter
      OnKeyPress = edtNMGRUPKeyPress
    end
    object GridGrupo: TDBGrid
      Left = 381
      Top = 190
      Width = 228
      Height = 115
      DataSource = dsGrupos
      Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 10
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      Visible = False
      OnExit = GridGrupoExit
      OnKeyDown = GridProdutosKeyDown
      OnKeyPress = GridGrupoKeyPress
      Columns = <
        item
          Expanded = False
          FieldName = 'GRU_CDGRUP'
          Title.Caption = 'C'#211'D.'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 33
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'GRU_NMGRUP'
          Title.Caption = 'GRUPO'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 364
          Visible = True
        end>
    end
    object GridProdutos: TDBGrid
      Left = 161
      Top = 315
      Width = 384
      Height = 54
      DataSource = dsProdutos
      Options = [dgTitles, dgIndicator, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
      TabOrder = 18
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'MS Sans Serif'
      TitleFont.Style = []
      OnKeyDown = GridProdutosKeyDown
      Columns = <
        item
          Expanded = False
          FieldName = 'PRO_CDPROD'
          Title.Caption = 'C'#211'DIGO'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 96
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRO_NMPROD'
          Title.Caption = 'DESCRI'#199#195'O'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 364
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRO_NMFABR'
          Title.Caption = 'FABRICANTE'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 186
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRO_VLVEND'
          Title.Caption = 'VL_VENDA'
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -11
          Title.Font.Name = 'MS Sans Serif'
          Title.Font.Style = [fsBold]
          Width = 91
          Visible = True
        end>
    end
  end
  object Panel2: TPanel
    Left = 0
    Top = 423
    Width = 638
    Height = 41
    Align = alBottom
    TabOrder = 2
    object btAdicionar: TBitBtn
      Left = 8
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Adicionar'
      TabOrder = 0
      OnClick = btAdicionarClick
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
    end
    object btEditar: TBitBtn
      Left = 88
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Editar'
      TabOrder = 1
      OnClick = btEditarClick
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
    end
    object btExcluir: TBitBtn
      Left = 168
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Excluir'
      TabOrder = 2
      OnClick = btExcluirClick
      Glyph.Data = {
        
        0E0E0E0E0E0E0B08171B0C1B1B1B1B1B1B1B1B1B1B1B1B1B1B0C}
    end
    object btLocalizar: TBitBtn
      Left = 315
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Localizar'
      TabOrder = 3
      OnClick = btLocalizarClick
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
    end
    object btGravar: TBitBtn
      Left = 592
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Gravar'
      TabOrder = 4
      OnClick = btGravarClick
      Glyph.Data = {
        
        1311110F0E0C0A081B1F121F1F1F1F1F1F1F1F1F1F1F1F1F1F12}
    end
    object btCancelar: TBitBtn
      Left = 676
      Top = 8
      Width = 75
      Height = 25
      Caption = 'Cancelar'
      TabOrder = 5
      OnClick = btCancelarClick
      Glyph.Data = {
        
        1311110F0E0C0A081B1F121F1F1F1F1F1F1F1F1F1F1F1F1F1F12}
    end
  end
  object dsProdutos: TDataSource
    AutoEdit = False
    DataSet = dmInfoADM.tbProdutos
    OnStateChange = dsProdutosStateChange
    OnDataChange = dsProdutosDataChange
    Left = 304
    Top = 40
  end
  object dsFornecedor: TDataSource
    DataSet = dmInfoADM.tbFornedores
    Left = 392
    Top = 40
  end
  object dsGrupos: TDataSource
    DataSet = dmInfoADM.qryGrupos
    Left = 432
    Top = 40
  end
  object dsSecoes: TDataSource
    DataSet = dmInfoADM.tbSecao
    Left = 472
    Top = 40
  end
  object dsUnidade: TDataSource
    DataSet = dmInfoADM.tbUnidade
    Left = 512
    Top = 40
  end
end
